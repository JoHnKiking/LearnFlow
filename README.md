...
│   └── app.json              # Expo应用配置
│  
├── server/                   # Node.js后端服务（MySQL数据持久化）
│   ├── src/
│   │   ├── config/           # 配置管理
│   │   │   └── database.ts      # 数据库配置和连接管理
│   │   ├── controllers/      # 控制器层（API端点处理）
│   │   │   ├── authController.ts # 用户认证相关API控制器
│   │   │   ├── skillController.ts  # 技能树相关API控制器
│   │   │   └── index.ts          # 控制器统一导出
│   │   ├── models/           # 数据层（MySQL实体）
│   │   │   ├── User.ts           # 用户数据
│   │   │   ├── SkillTree.ts      # 技能树数据
│   │   │   ├── LearningRecord.ts # 学习记录数据
│   │   │   ├── PopularDomain.ts  # 热门领域数据
│   │   │   ├── DeviceSession.ts   # 设备会话数据
│   │   │   └── index.ts          # 数据统一导出
│   │   ├── routes/           # 路由层（URL映射）
│   │   │   ├── authRoutes.ts     # 用户认证相关路由配置
│   │   │   ├── skillRoutes.ts    # 技能树相关路由配置
│   │   │   └── index.ts          # 路由统一入口
│   │   ├── services/         # 服务层（业务逻辑+数据访问）
│   │   │   ├── authService.ts    # 用户认证业务逻辑
│   │   │   ├── skillService.ts   # 技能树业务逻辑
│   │   │   ├── databaseService.ts # MySQL数据库服务
│   │   │   └── index.ts          # 服务统一导出
│   │   ├── types/            # 类型定义层
│   │   │   ├── skill.ts          # 技能树相关类型
│   │   │   └── index.ts          # 类型统一导出
│   │   └── app.ts            # 应用入口和Express配置
│   ├── sql/                  # SQL脚本目录
│   │   └── init_database.sql     # 数据库初始化脚本
│   ├── scripts/              # 工具脚本目录
│   │   └── init-database.js      # 数据库初始化工具
│   ├── package.json          # 项目依赖配置（包含mysql2）
│   └── tsconfig.json         # TypeScript编译配置
├── .gitignore                # Git忽略文件配置
...
用户打开App → 显示登录界面 → 选择登录方式
    ↓
微信登录：调用微信SDK获取授权码 → 发送到服务端 → 返回用户信息和令牌
    ↓
手机号登录：输入手机号和密码 → 发送到服务端 → 返回用户信息和令牌
    ↓
保存用户信息和设备会话 → 进入主界面
